...
static inline uint32_t rotl(uint32_t value, int shift)
{
  return (value << shift) | (value >> (32 - shift));
}

static inline void s20_quarterround(uint32_t *y0, uint32_t *y1, uint32_t *y2, uint32_t *y3)
{
  *y1 = *y1 ^ rotl(*y0 + *y3, 7);
  *y2 = *y2 ^ rotl(*y1 + *y0, 9);
  *y3 = *y3 ^ rotl(*y2 + *y1, 13);
...
static inline void s20_rowround(uint32_t y[16])
{
...
static inline void s20_columnround(uint32_t x[16])
{
...
static inline void s20_doubleround(uint32_t x[16])
{
  s20_columnround(x);
  s20_rowround(x);
}
...
static inline uint32_t s20_littleendian(uint8_t *b)
{
  return b[0] +
         ((uint_fast16_t) b[1] << 8) +
...
static inline void s20_rev_littleendian(uint8_t *b, uint32_t w)
{
  b[0] = w;
  b[1] = w >> 8;
  b[2] = w >> 16;
...
static inline void s20_hash(uint8_t seq[64])
{
  int i;
  uint32_t x[16];
  uint32_t z[16];
...
static inline void s20_expand16(uint8_t *k,
                         uint8_t n[16],
...
static inline void s20_expand32(uint8_t *k,
                         uint8_t n[16],
                         uint8_t keystream[64])
{
  int i, j;
